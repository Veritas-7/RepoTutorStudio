...
import type { IncrementalReport, SourceSnapshotReport, StudySession } from "@repotutor/shared";
import { pathExists } from "./fs-utils.js";
import { readJson } from "./storage.js";
import { listSessions } from "./sessions.js";

export async function findPreviousSnapshot(studiesRoot: string, current: StudySession): Promise<{ session: StudySession; snapshot: SourceSnapshotReport } | null> {
...
export function buildIncrementalReport(current: SourceSnapshotReport, previous: { session: StudySession; snapshot: SourceSnapshotReport } | null): IncrementalReport {
  if (!previous) {
    return {
