...
export function renderStudyHtml(input: StudyHtmlInput): RenderedStudy {
  ...
  const pages: RenderedPage[] = [
    {
      ...
      html: pageShell("학습 리포트", "index.html", `
        ...
        <section class="grid">
          ...
          <article><h3>Pipeline Orchestration Readiness</h3><p>${escapeHtml(input.pipelineOrchestrationReadinessReport.summary)}</p><p>Airflow, Dagster, Prefect 패턴으로 DAG/job/flow, task, schedule, sensor, asset, partition, executor, deployment 준비도를 정리합니다.</p><a href="pipeline-orchestration-readiness.html">Pipeline Orchestration 열기</a></article>
          ...
          <article><h3>API Gateway Readiness</h3><p>${escapeHtml(input.apiGatewayReadinessReport.summary)}</p><p>Kong, Tyk, KrakenD 패턴으로 route, upstream, auth, plugin, traffic policy, validation 준비도를 정리합니다.</p><a href="api-gateway-readiness.html">API Gateway 열기</a></article>
    ...
    {
      name: "pipeline-orchestration-readiness.html",
      title: "Pipeline Orchestration Readiness",
      html: pageShell("Pipeline Orchestration Readiness", "pipeline-orchestration-readiness.html", `<section class="panel" data-source-pattern="PipelineOrchestration"><h2>Pipeline Orchestration Snapshot</h2><p>${escapeHtml(input.pipelineOrchestrationReadinessReport.summary)}</p><p class="muted">${escapeHtml(input.pipelineOrchestrationReadinessReport.sourcePattern)}</p><dl class="meta"><div><dt>setups</dt><dd>${input.pipelineOrchestrationReadinessReport.pipelineOrchestrationSetups.length}</dd></div><div><dt>orchestrators</dt><dd>${input.pipelineOrchestrationReadinessReport.orchestratorSignals.length}</dd></div><div><dt>schedules</dt><dd>${input.pipelineOrchestrationReadinessReport.scheduleSignals.length}</dd></div><div><dt>reliability</dt><dd>${input.pipelineOrchestrationReadinessReport.reliabilitySignals.length}</dd></div></dl><p class="muted">RepoTutor records pipeline orchestration readiness only. It does not run Airflow, Dagster, Prefect, schedulers, executors, workers, backfills, deployments, or CI commands.</p></section><section class="grid"><article class="pipeline-orchestration-readiness-card"><h3>Pipeline Orchestration Setups</h3>${pipelineOrchestrationReadinessSetupList(input.pipelineOrchestrationReadinessReport.pipelineOrchestrationSetups)}</article><article class="pipeline-orchestration-readiness-card"><h3>Orchestrator Signals</h3>${pipelineOrchestrationReadinessSignalList(input.pipelineOrchestrationReadinessReport.orchestratorSignals, "signal")}</article><article class="pipeline-orchestration-readiness-card"><h3>DAG Signals</h3>${pipelineOrchestrationReadinessSignalList(input.pipelineOrchestrationReadinessReport.dagSignals, "signal")}</article><article class="pipeline-orchestration-readiness-card"><h3>Task Signals</h3>${pipelineOrchestrationReadinessSignalList(input.pipelineOrchestrationReadinessReport.taskSignals, "signal")}</article></section><section class="grid"><article class="pipeline-orchestration-readiness-card"><h3>Dependency Signals</h3>${pipelineOrchestrationReadinessSignalList(input.pipelineOrchestrationReadinessReport.dependencySignals, "signal")}</article><article class="pipeline-orchestration-readiness-card"><h3>Schedule Signals</h3>${pipelineOrchestrationReadinessSignalList(input.pipelineOrchestrationReadinessReport.scheduleSignals, "signal")}</article><article class="pipeline-orchestration-readiness-card"><h3>Sensor Signals</h3>${pipelineOrchestrationReadinessSignalList(input.pipelineOrchestrationReadinessReport.sensorSignals, "signal")}</article><article class="pipeline-orchestration-readiness-card"><h3>Asset Signals</h3>${pipelineOrchestrationReadinessSignalList(input.pipelineOrchestrationReadinessReport.assetSignals, "signal")}</article></section><section class="grid"><article class="pipeline-orchestration-readiness-card"><h3>Partition Signals</h3>${pipelineOrchestrationReadinessSignalList(input.pipelineOrchestrationReadinessReport.partitionSignals, "signal")}</article><article class="pipeline-orchestration-readiness-card"><h3>Reliability Signals</h3>${pipelineOrchestrationReadinessSignalList(input.pipelineOrchestrationReadinessReport.reliabilitySignals, "signal")}</article><article class="pipeline-orchestration-readiness-card"><h3>Executor Signals</h3>${pipelineOrchestrationReadinessSignalList(input.pipelineOrchestrationReadinessReport.executorSignals, "signal")}</article><article class="pipeline-orchestration-readiness-card"><h3>Deployment Signals</h3>${pipelineOrchestrationReadinessSignalList(input.pipelineOrchestrationReadinessReport.deploymentSignals, "signal")}</article></section><section class="grid"><article class="pipeline-orchestration-readiness-card"><h3>Observability Signals</h3>${pipelineOrchestrationReadinessSignalList(input.pipelineOrchestrationReadinessReport.observabilitySignals, "signal")}</article><article class="pipeline-orchestration-readiness-card"><h3>CI Signals</h3>${pipelineOrchestrationReadinessSignalList(input.pipelineOrchestrationReadinessReport.ciSignals, "signal")}</article><article class="pipeline-orchestration-readiness-card"><h3>Package Signals</h3>${pipelineOrchestrationReadinessSignalList(input.pipelineOrchestrationReadinessReport.packageSignals, "signal")}</article><article class="pipeline-orchestration-readiness-card"><h3>Recommended Commands</h3>${pipelineOrchestrationReadinessCommandList(input.pipelineOrchestrationReadinessReport.recommendedCommands)}</article><article class="pipeline-orchestration-readiness-card"><h3>Risk Queue</h3>${pipelineOrchestrationReadinessRiskList(input.pipelineOrchestrationReadinessReport.riskQueue)}</article><article class="pipeline-orchestration-readiness-card"><h3>다음 확인 단계</h3>${list(input.pipelineOrchestrationReadinessReport.learnerNextSteps)}</article></section>`, input)
    },
    {
      name: "service-mesh-readiness.html",
      title: "Service Mesh Readiness",
      html: pageShell("Service Mesh Readiness", "service-mesh-readiness.html", `<section class="panel" data-source-pattern="ServiceMesh"><h2>Service Mesh Snapshot</h2><p>${escapeHtml(input.serviceMeshReadinessReport.summary)}</p><p class="muted">${escapeHtml(input.serviceMeshReadinessReport.sourcePattern)}</p><dl class="meta"><div><dt>setups</dt><dd>${input.serviceMeshReadinessReport.serviceMeshSetups.length}</dd></div><div><dt>mesh</dt><dd>${input.serviceMeshReadinessReport.meshSignals.length}</dd></div><div><dt>traffic</dt><dd>${input.serviceMeshReadinessReport.trafficSignals.length}</dd></div><div><dt>security</dt><dd>${input.serviceMeshReadinessReport.securitySignals.length}</dd></div><div><dt>telemetry</dt><dd>${input.serviceMeshReadinessReport.telemetrySignals.length}</dd></div></dl><p class="muted">RepoTutor records service mesh readiness only. It does not run istioctl, linkerd, consul, envoy, Kubernetes, Helm, traffic, proxy, policy, or CI commands.</p></section><section class="grid"><article class="service-mesh-readiness-card"><h3>Service Mesh Setups</h3>${serviceMeshReadinessSetupList(input.serviceMeshReadinessReport.serviceMeshSetups)}</article><article class="service-mesh-readiness-card"><h3>Mesh Signals</h3>${serviceMeshReadinessSignalList(input.serviceMeshReadinessReport.meshSignals, "signal")}</article><article class="service-mesh-readiness-card"><h3>Control Plane Signals</h3>${serviceMeshReadinessSignalList(input.serviceMeshReadinessReport.controlPlaneSignals, "signal")}</article><article class="service-mesh-readiness-card"><h3>Injection Signals</h3>${serviceMeshReadinessSignalList(input.serviceMeshReadinessReport.injectionSignals, "signal")}</article></section><section class="grid"><article class="service-mesh-readiness-card"><h3>Traffic Signals</h3>${serviceMeshReadinessSignalList(input.serviceMeshReadinessReport.trafficSignals, "signal")}</article><article class="service-mesh-readiness-card"><h3>Security Signals</h3>${serviceMeshReadinessSignalList(input.serviceMeshReadinessReport.securitySignals, "signal")}</article><article class="service-mesh-readiness-card"><h3>mTLS Signals</h3>${serviceMeshReadinessSignalList(input.serviceMeshReadinessReport.mtlsSignals, "signal")}</article><article class="service-mesh-readiness-card"><h3>Resilience Signals</h3>${serviceMeshReadinessSignalList(input.serviceMeshReadinessReport.resilienceSignals, "signal")}</article></section><section class="grid"><article class="service-mesh-readiness-card"><h3>Gateway Signals</h3>${serviceMeshReadinessSignalList(input.serviceMeshReadinessReport.gatewaySignals, "signal")}</article><article class="service-mesh-readiness-card"><h3>Telemetry Signals</h3>${serviceMeshReadinessSignalList(input.serviceMeshReadinessReport.telemetrySignals, "signal")}</article><article class="service-mesh-readiness-card"><h3>Multicluster Signals</h3>${serviceMeshReadinessSignalList(input.serviceMeshReadinessReport.multiclusterSignals, "signal")}</article><article class="service-mesh-readiness-card"><h3>CI Signals</h3>${serviceMeshReadinessSignalList(input.serviceMeshReadinessReport.ciSignals, "signal")}</article><article class="service-mesh-readiness-card"><h3>Package Signals</h3>${serviceMeshReadinessSignalList(input.serviceMeshReadinessReport.packageSignals, "signal")}</article><article class="service-mesh-readiness-card"><h3>Recommended Commands</h3>${serviceMeshReadinessCommandList(input.serviceMeshReadinessReport.recommendedCommands)}</article><article class="service-mesh-readiness-card"><h3>Risk Queue</h3>${serviceMeshReadinessRiskList(input.serviceMeshReadinessReport.riskQueue)}</article><article class="service-mesh-readiness-card"><h3>다음 확인 단계</h3>${list(input.serviceMeshReadinessReport.learnerNextSteps)}</article></section>`, input)
    },
    {
      name: "ingress-controller-readiness.html",
      title: "Ingress Controller Readiness",
      html: pageShell("Ingress Controller Readiness", "ingress-controller-readiness.html", `<section class="panel" data-source-pattern="IngressController"><h2>Ingress Controller Snapshot</h2><p>${escapeHtml(input.ingressControllerReadinessReport.summary)}</p><p class="muted">${escapeHtml(input.ingressControllerReadinessReport.sourcePattern)}</p><dl class="meta"><div><dt>setups</dt><dd>${input.ingressControllerReadinessReport.ingressControllerSetups.length}</dd></div><div><dt>controllers</dt><dd>${input.ingressControllerReadinessReport.controllerSignals.length}</dd></div><div><dt>routes</dt><dd>${input.ingressControllerReadinessReport.routeSignals.length}</dd></div><div><dt>TLS</dt><dd>${input.ingressControllerReadinessReport.tlsSignals.length}</dd></div><div><dt>admission</dt><dd>${input.ingressControllerReadinessReport.admissionSignals.length}</dd></div></dl><p class="muted">RepoTutor records ingress controller readiness only. It does not run kubectl, helm, ingress controllers, Gateway API controllers, admission webhooks, traffic, DNS, TLS, load balancer, or CI commands.</p></section><section class="grid"><article class="ingress-controller-readiness-card"><h3>Ingress Controller Setups</h3>${ingressControllerReadinessSetupList(input.ingressControllerReadinessReport.ingressControllerSetups)}</article><article class="ingress-controller-readiness-card"><h3>Controller Signals</h3>${ingressControllerReadinessSignalList(input.ingressControllerReadinessReport.controllerSignals, "signal")}</article><article class="ingress-controller-readiness-card"><h3>Ingress Class Signals</h3>${ingressControllerReadinessSignalList(input.ingressControllerReadinessReport.ingressClassSignals, "signal")}</article><article class="ingress-controller-readiness-card"><h3>Route Signals</h3>${ingressControllerReadinessSignalList(input.ingressControllerReadinessReport.routeSignals, "signal")}</article></section><section class="grid"><article class="ingress-controller-readiness-card"><h3>Service Exposure Signals</h3>${ingressControllerReadinessSignalList(input.ingressControllerReadinessReport.serviceExposureSignals, "signal")}</article><article class="ingress-controller-readiness-card"><h3>TLS Signals</h3>${ingressControllerReadinessSignalList(input.ingressControllerReadinessReport.tlsSignals, "signal")}</article><article class="ingress-controller-readiness-card"><h3>Middleware Signals</h3>${ingressControllerReadinessSignalList(input.ingressControllerReadinessReport.middlewareSignals, "signal")}</article><article class="ingress-controller-readiness-card"><h3>Policy Signals</h3>${ingressControllerReadinessSignalList(input.ingressControllerReadinessReport.policySignals, "signal")}</article></section><section class="grid"><article class="ingress-controller-readiness-card"><h3>Load Balancing Signals</h3>${ingressControllerReadinessSignalList(input.ingressControllerReadinessReport.loadBalancingSignals, "signal")}</article><article class="ingress-controller-readiness-card"><h3>Observability Signals</h3>${ingressControllerReadinessSignalList(input.ingressControllerReadinessReport.observabilitySignals, "signal")}</article><article class="ingress-controller-readiness-card"><h3>Admission Signals</h3>${ingressControllerReadinessSignalList(input.ingressControllerReadinessReport.admissionSignals, "signal")}</article><article class="ingress-controller-readiness-card"><h3>CI Signals</h3>${ingressControllerReadinessSignalList(input.ingressControllerReadinessReport.ciSignals, "signal")}</article><article class="ingress-controller-readiness-card"><h3>Package Signals</h3>${ingressControllerReadinessSignalList(input.ingressControllerReadinessReport.packageSignals, "signal")}</article><article class="ingress-controller-readiness-card"><h3>Recommended Commands</h3>${ingressControllerReadinessCommandList(input.ingressControllerReadinessReport.recommendedCommands)}</article><article class="ingress-controller-readiness-card"><h3>Risk Queue</h3>${ingressControllerReadinessRiskList(input.ingressControllerReadinessReport.riskQueue)}</article><article class="ingress-controller-readiness-card"><h3>다음 확인 단계</h3>${list(input.ingressControllerReadinessReport.learnerNextSteps)}</article></section>`, input)
    },
    {
      name: "dns-readiness.html",
      title: "DNS Readiness",
    ...
    {
      name: "helm-readiness.html",
      title: "Helm Readiness",
    ...
    {
      name: "api-gateway-readiness.html",
      title: "API Gateway Readiness",
      html: pageShell("API Gateway Readiness", "api-gateway-readiness.html", `<section class="panel" data-source-pattern="ApiGateway"><h2>API Gateway Snapshot</h2><p>${escapeHtml(input.apiGatewayReadinessReport.summary)}</p><p class="muted">${escapeHtml(input.apiGatewayReadinessReport.sourcePattern)}</p><dl class="meta"><div><dt>setups</dt><dd>${input.apiGatewayReadinessReport.apiGatewaySetups.length}</dd></div><div><dt>gateways</dt><dd>${input.apiGatewayReadinessReport.gatewaySignals.length}</dd></div><div><dt>routes</dt><dd>${input.apiGatewayReadinessReport.routeSignals.length}</dd></div><div><dt>auth</dt><dd>${input.apiGatewayReadinessReport.authSignals.length}</dd></div><div><dt>traffic</dt><dd>${input.apiGatewayReadinessReport.trafficPolicySignals.length}</dd></div></dl><p class="muted">RepoTutor records API gateway readiness only. It does not start gateways, proxy requests, contact admin APIs, mutate route configs, run plugins, call cloud APIs, or execute gateway CLIs.</p></section><section class="grid"><article class="api-gateway-readiness-card"><h3>API Gateway Setups</h3>${apiGatewayReadinessSetupList(input.apiGatewayReadinessReport.apiGatewaySetups)}</article><article class="api-gateway-readiness-card"><h3>Gateway Signals</h3>${apiGatewayReadinessSignalList(input.apiGatewayReadinessReport.gatewaySignals, "signal")}</article><article class="api-gateway-readiness-card"><h3>Route Signals</h3>${apiGatewayReadinessSignalList(input.apiGatewayReadinessReport.routeSignals, "signal")}</article><article class="api-gateway-readiness-card"><h3>Upstream Signals</h3>${apiGatewayReadinessSignalList(input.apiGatewayReadinessReport.upstreamSignals, "signal")}</article></section><section class="grid"><article class="api-gateway-readiness-card"><h3>Auth Signals</h3>${apiGatewayReadinessSignalList(input.apiGatewayReadinessReport.authSignals, "signal")}</article><article class="api-gateway-readiness-card"><h3>Plugin Signals</h3>${apiGatewayReadinessSignalList(input.apiGatewayReadinessReport.pluginSignals, "signal")}</article><article class="api-gateway-readiness-card"><h3>Traffic Policy Signals</h3>${apiGatewayReadinessSignalList(input.apiGatewayReadinessReport.trafficPolicySignals, "signal")}</article><article class="api-gateway-readiness-card"><h3>Observability Signals</h3>${apiGatewayReadinessSignalList(input.apiGatewayReadinessReport.observabilitySignals, "signal")}</article></section><section class="grid"><article class="api-gateway-readiness-card"><h3>Validation Signals</h3>${apiGatewayReadinessSignalList(input.apiGatewayReadinessReport.validationSignals, "signal")}</article><article class="api-gateway-readiness-card"><h3>CI Signals</h3>${apiGatewayReadinessSignalList(input.apiGatewayReadinessReport.ciSignals, "signal")}</article><article class="api-gateway-readiness-card"><h3>Package Signals</h3>${apiGatewayReadinessSignalList(input.apiGatewayReadinessReport.packageSignals, "signal")}</article><article class="api-gateway-readiness-card"><h3>Recommended Commands</h3>${apiGatewayReadinessCommandList(input.apiGatewayReadinessReport.recommendedCommands)}</article><article class="api-gateway-readiness-card"><h3>Risk Queue</h3>${apiGatewayReadinessRiskList(input.apiGatewayReadinessReport.riskQueue)}</article><article class="api-gateway-readiness-card"><h3>다음 확인 단계</h3>${list(input.apiGatewayReadinessReport.learnerNextSteps)}</article></section>`, input)
    },
    {
  ...
  const manifest: HtmlExportManifest = {
    ...
    entrypoints: [
      ...
      { label: "API Gateway Readiness", path: "html/api-gateway-readiness.html", description: "Kong/Tyk/KrakenD식 route, upstream, auth, plugin, traffic policy, validation 준비도를 확인합니다." },
...
function learningPathFor(input: StudyHtmlInput): Array<{ title: string; href: string; goal: string; evidence: string }> {
  return [
    ...
    {
      title: "Pipeline orchestration readiness 확인",
      href: "pipeline-orchestration-readiness.html",
      goal: "Airflow/Dagster/Prefect식 DAG/job/flow, task, schedule, sensor, asset/partition, executor/deployment 흐름을 보고 data pipeline orchestration 관문을 확인합니다.",
      evidence: `pipeline orchestration setups ${input.pipelineOrchestrationReadinessReport.pipelineOrchestrationSetups.length}개, orchestrator signals ${input.pipelineOrchestrationReadinessReport.orchestratorSignals.length}개`
    },
    {
      title: "Service mesh readiness 확인",
      href: "service-mesh-readiness.html",
      goal: "Istio/Linkerd/Consul식 control plane, proxy injection, traffic policy, mTLS, gateway, telemetry 흐름을 보고 service mesh 관문을 확인합니다.",
      evidence: `service mesh setups ${input.serviceMeshReadinessReport.serviceMeshSetups.length}개, mesh signals ${input.serviceMeshReadinessReport.meshSignals.length}개`
    },
    {
      title: "Ingress controller readiness 확인",
      href: "ingress-controller-readiness.html",
      goal: "ingress-nginx/Traefik/Envoy Gateway식 class, route, exposure, TLS, middleware/policy, admission, observability 관문을 확인합니다.",
      evidence: `ingress controller setups ${input.ingressControllerReadinessReport.ingressControllerSetups.length}개, route signals ${input.ingressControllerReadinessReport.routeSignals.length}개`
    },
    {
      title: "DNS readiness 확인",
    ...
    {
      title: "API gateway readiness 확인",
      href: "api-gateway-readiness.html",
      goal: "Kong/Tyk/KrakenD식 route, upstream, auth, plugin, traffic policy, validation 관문을 확인합니다.",
      evidence: `API gateway setups ${input.apiGatewayReadinessReport.apiGatewaySetups.length}개, traffic policy signals ${input.apiGatewayReadinessReport.trafficPolicySignals.length}개`
    },
    {
...
function dastReadinessSetupList(items: DastReadinessReport["dastSetups"]): string {
  if (items.length === 0) return "<p class=\"muted\">DAST setup이 없습니다.</p>";
  return `<ul>${items.map((item) => `<li><strong>${escapeHtml(item.filePath)}</strong> [${escapeHtml(item.tool)}/${escapeHtml(item.readiness)}]<br>target/crawl/active/auth/template/safety/output/CI/findings ${item.targetCount}/${item.crawlerCount}/${item.activeScanCount}/${item.authCount}/${item.templateCount}/${item.safetyCount}/${item.outputCount}/${item.ciCount}/${item.findingCount}<br>${escapeHtml(item.evidence)}<br><a href="${escapeHtml(dastReadinessHref(item.sourceHref))}">원본 열기</a></li>`).join("")}</ul>`;
}

function dastReadinessSignalList<T extends string>(items: Array<Record<T, string> & { readiness: string; evidence: string; relatedHref: string }>, labelKey: T): string {
  if (items.length === 0) return "<p class=\"muted\">DAST signal이 없습니다.</p>";
  return `<ul>${items.map((item) => `<li><strong>${escapeHtml(item[labelKey])}</strong> [${escapeHtml(item.readiness)}]<br>${escapeHtml(item.evidence)}<br><a href="${escapeHtml(dastReadinessHref(item.relatedHref))}">관련 페이지 열기</a></li>`).join("")}</ul>`;
}

function dastReadinessCommandList(items: DastReadinessReport["recommendedCommands"]): string {
  if (items.length === 0) return "<p class=\"muted\">recommended command가 없습니다.</p>";
  return `<ul>${items.map((item) => `<li><code>${escapeHtml(item.command)}</code><br>${escapeHtml(item.purpose)}</li>`).join("")}</ul>`;
}

function dastReadinessRiskList(items: DastReadinessReport["riskQueue"]): string {
  if (items.length === 0) return "<p class=\"muted\">risk queue가 없습니다.</p>";
  return `<ul>${items.map((item) => `<li><strong>${escapeHtml(item.priority)}</strong>: ${escapeHtml(item.action)}<br><span class="muted">${escapeHtml(item.why)}</span><br><a href="${escapeHtml(dastReadinessHref(item.relatedHref))}">관련 페이지 열기</a></li>`).join("")}</ul>`;
}
...
function pipelineOrchestrationReadinessSetupList(items: PipelineOrchestrationReadinessReport["pipelineOrchestrationSetups"]): string {
  if (items.length === 0) return "<p class=\"muted\">pipeline orchestration setup이 없습니다.</p>";
  return `<ul>${items.map((item) => `<li><strong>${escapeHtml(item.filePath)}</strong> [${escapeHtml(item.orchestrator)}/${escapeHtml(item.readiness)}]<br>DAG/task/dependency/schedule/sensor/asset/partition/retry/backfill/executor/deploy/observability/CI ${item.dagCount}/${item.taskCount}/${item.dependencyCount}/${item.scheduleCount}/${item.sensorCount}/${item.assetCount}/${item.partitionCount}/${item.retryCount}/${item.backfillCount}/${item.executorCount}/${item.deploymentCount}/${item.observabilityCount}/${item.ciCount}<br>${escapeHtml(item.evidence)}<br><a href="${escapeHtml(pipelineOrchestrationReadinessHref(item.sourceHref))}">원본 열기</a></li>`).join("")}</ul>`;
}

function pipelineOrchestrationReadinessSignalList<T extends string>(items: Array<Record<T, string> & { readiness: string; evidence: string; relatedHref: string }>, labelKey: T): string {
  if (items.length === 0) return "<p class=\"muted\">pipeline orchestration signal이 없습니다.</p>";
  return `<ul>${items.map((item) => `<li><strong>${escapeHtml(item[labelKey])}</strong> [${escapeHtml(item.readiness)}]<br>${escapeHtml(item.evidence)}<br><a href="${escapeHtml(pipelineOrchestrationReadinessHref(item.relatedHref))}">관련 페이지 열기</a></li>`).join("")}</ul>`;
}

function pipelineOrchestrationReadinessCommandList(items: PipelineOrchestrationReadinessReport["recommendedCommands"]): string {
  if (items.length === 0) return "<p class=\"muted\">recommended command가 없습니다.</p>";
  return `<ul>${items.map((item) => `<li><code>${escapeHtml(item.command)}</code><br>${escapeHtml(item.purpose)}</li>`).join("")}</ul>`;
}

function pipelineOrchestrationReadinessRiskList(items: PipelineOrchestrationReadinessReport["riskQueue"]): string {
  if (items.length === 0) return "<p class=\"muted\">risk queue가 없습니다.</p>";
  return `<ul>${items.map((item) => `<li><strong>${escapeHtml(item.priority)}</strong>: ${escapeHtml(item.action)}<br><span class="muted">${escapeHtml(item.why)}</span><br><a href="${escapeHtml(pipelineOrchestrationReadinessHref(item.relatedHref))}">관련 페이지 열기</a></li>`).join("")}</ul>`;
}

function pipelineOrchestrationReadinessHref(href: string): string {
  if (href.startsWith("source/")) return `../${href}`;
  return htmlPageHref(href);
}

function serviceMeshReadinessSetupList(items: ServiceMeshReadinessReport["serviceMeshSetups"]): string {
  if (items.length === 0) return "<p class=\"muted\">service mesh setup이 없습니다.</p>";
  return `<ul>${items.map((item) => `<li><strong>${escapeHtml(item.filePath)}</strong> [${escapeHtml(item.mesh)}/${escapeHtml(item.readiness)}]<br>control-plane/sidecar/gateway/route/traffic/security/mTLS/identity/telemetry/resilience/multicluster/CI ${item.controlPlaneCount}/${item.sidecarCount}/${item.gatewayCount}/${item.routeCount}/${item.trafficPolicyCount}/${item.securityPolicyCount}/${item.mtlsCount}/${item.identityCount}/${item.telemetryCount}/${item.resilienceCount}/${item.multiClusterCount}/${item.ciCount}<br>${escapeHtml(item.evidence)}<br><a href="${escapeHtml(serviceMeshReadinessHref(item.sourceHref))}">원본 열기</a></li>`).join("")}</ul>`;
}

function serviceMeshReadinessSignalList<T extends string>(items: Array<Record<T, string> & { readiness: string; evidence: string; relatedHref: string }>, labelKey: T): string {
  if (items.length === 0) return "<p class=\"muted\">service mesh signal이 없습니다.</p>";
  return `<ul>${items.map((item) => `<li><strong>${escapeHtml(item[labelKey])}</strong> [${escapeHtml(item.readiness)}]<br>${escapeHtml(item.evidence)}<br><a href="${escapeHtml(serviceMeshReadinessHref(item.relatedHref))}">관련 페이지 열기</a></li>`).join("")}</ul>`;
}

function serviceMeshReadinessCommandList(items: ServiceMeshReadinessReport["recommendedCommands"]): string {
  if (items.length === 0) return "<p class=\"muted\">recommended command가 없습니다.</p>";
  return `<ul>${items.map((item) => `<li><code>${escapeHtml(item.command)}</code><br>${escapeHtml(item.purpose)}</li>`).join("")}</ul>`;
}

function serviceMeshReadinessRiskList(items: ServiceMeshReadinessReport["riskQueue"]): string {
  if (items.length === 0) return "<p class=\"muted\">risk queue가 없습니다.</p>";
  return `<ul>${items.map((item) => `<li><strong>${escapeHtml(item.priority)}</strong>: ${escapeHtml(item.action)}<br><span class="muted">${escapeHtml(item.why)}</span><br><a href="${escapeHtml(serviceMeshReadinessHref(item.relatedHref))}">관련 페이지 열기</a></li>`).join("")}</ul>`;
}

function serviceMeshReadinessHref(href: string): string {
  if (href.startsWith("source/")) return `../${href}`;
  return htmlPageHref(href);
}

function ingressControllerReadinessSetupList(items: IngressControllerReadinessReport["ingressControllerSetups"]): string {
  if (items.length === 0) return "<p class=\"muted\">ingress controller setup이 없습니다.</p>";
  return `<ul>${items.map((item) => `<li><strong>${escapeHtml(item.filePath)}</strong> [${escapeHtml(item.controller)}/${escapeHtml(item.readiness)}]<br>controller/class/route/exposure/TLS ${item.controllerCount}/${item.ingressClassCount}/${item.routeCount}/${item.serviceExposureCount}/${item.tlsCount}<br>middleware/policy/load-balancing/observability/admission/CI ${item.middlewareCount}/${item.policyCount}/${item.loadBalancingCount}/${item.observabilityCount}/${item.admissionCount}/${item.ciCount}<br>${escapeHtml(item.evidence)}<br><a href="${escapeHtml(ingressControllerReadinessHref(item.sourceHref))}">원본 열기</a></li>`).join("")}</ul>`;
}

function ingressControllerReadinessSignalList<T extends string>(items: Array<Record<T, string> & { readiness: string; evidence: string; relatedHref: string }>, labelKey: T): string {
  if (items.length === 0) return "<p class=\"muted\">ingress controller signal이 없습니다.</p>";
  return `<ul>${items.map((item) => `<li><strong>${escapeHtml(item[labelKey])}</strong> [${escapeHtml(item.readiness)}]<br>${escapeHtml(item.evidence)}<br><a href="${escapeHtml(ingressControllerReadinessHref(item.relatedHref))}">관련 페이지 열기</a></li>`).join("")}</ul>`;
}

function ingressControllerReadinessCommandList(items: IngressControllerReadinessReport["recommendedCommands"]): string {
  if (items.length === 0) return "<p class=\"muted\">recommended command가 없습니다.</p>";
  return `<ul>${items.map((item) => `<li><code>${escapeHtml(item.command)}</code><br>${escapeHtml(item.purpose)}</li>`).join("")}</ul>`;
}

function ingressControllerReadinessRiskList(items: IngressControllerReadinessReport["riskQueue"]): string {
  if (items.length === 0) return "<p class=\"muted\">risk queue가 없습니다.</p>";
  return `<ul>${items.map((item) => `<li><strong>${escapeHtml(item.priority)}</strong>: ${escapeHtml(item.action)}<br><span class="muted">${escapeHtml(item.why)}</span><br><a href="${escapeHtml(ingressControllerReadinessHref(item.relatedHref))}">관련 페이지 열기</a></li>`).join("")}</ul>`;
}

function ingressControllerReadinessHref(href: string): string {
  if (href.startsWith("source/")) return `../${href}`;
  return htmlPageHref(href);
}

function dnsReadinessSetupList(items: DnsReadinessReport["dnsSetups"]): string {
  if (items.length === 0) return "<p class=\"muted\">DNS setup이 없습니다.</p>";
  return `<ul>${items.map((item) => `<li><strong>${escapeHtml(item.filePath)}</strong> [${escapeHtml(item.platform)}/${escapeHtml(item.readiness)}]<br>source/provider/zone/record/ownership/policy ${item.sourceCount}/${item.providerCount}/${item.zoneCount}/${item.recordCount}/${item.ownershipCount}/${item.policyCount}<br>CoreDNS/automation/observability/CI ${item.coreDnsCount}/${item.automationCount}/${item.observabilityCount}/${item.ciCount}<br>${escapeHtml(item.evidence)}<br><a href="${escapeHtml(dnsReadinessHref(item.sourceHref))}">원본 열기</a></li>`).join("")}</ul>`;
}

function dnsReadinessSignalList<T extends string>(items: Array<Record<T, string> & { readiness: string; evidence: string; relatedHref: string }>, labelKey: T): string {
  if (items.length === 0) return "<p class=\"muted\">DNS signal이 없습니다.</p>";
  return `<ul>${items.map((item) => `<li><strong>${escapeHtml(item[labelKey])}</strong> [${escapeHtml(item.readiness)}]<br>${escapeHtml(item.evidence)}<br><a href="${escapeHtml(dnsReadinessHref(item.relatedHref))}">관련 페이지 열기</a></li>`).join("")}</ul>`;
}

function dnsReadinessCommandList(items: DnsReadinessReport["recommendedCommands"]): string {
  if (items.length === 0) return "<p class=\"muted\">recommended command가 없습니다.</p>";
  return `<ul>${items.map((item) => `<li><code>${escapeHtml(item.command)}</code><br>${escapeHtml(item.purpose)}</li>`).join("")}</ul>`;
}

function dnsReadinessRiskList(items: DnsReadinessReport["riskQueue"]): string {
  if (items.length === 0) return "<p class=\"muted\">risk queue가 없습니다.</p>";
  return `<ul>${items.map((item) => `<li><strong>${escapeHtml(item.priority)}</strong>: ${escapeHtml(item.action)}<br><span class="muted">${escapeHtml(item.why)}</span><br><a href="${escapeHtml(dnsReadinessHref(item.relatedHref))}">관련 페이지 열기</a></li>`).join("")}</ul>`;
}
...
function helmReadinessSetupList(items: HelmReadinessReport["helmSetups"]): string {
  if (items.length === 0) return "<p class=\"muted\">Helm setup이 없습니다.</p>";
  return `<ul>${items.map((item) => `<li><strong>${escapeHtml(item.filePath)}</strong> [${escapeHtml(item.chartType)}/${escapeHtml(item.readiness)}]<br>chart/values/templates/dependencies/schema ${item.chartCount}/${item.valuesCount}/${item.templateCount}/${item.dependencyCount}/${item.schemaCount}<br>test/packaging/release/provenance/CI ${item.testCount}/${item.packagingCount}/${item.releaseCount}/${item.provenanceCount}/${item.ciCount}<br>${escapeHtml(item.evidence)}<br><a href="${escapeHtml(helmReadinessHref(item.sourceHref))}">원본 열기</a></li>`).join("")}</ul>`;
}

function helmReadinessSignalList<T extends string>(items: Array<Record<T, string> & { readiness: string; evidence: string; relatedHref: string }>, labelKey: T): string {
  if (items.length === 0) return "<p class=\"muted\">Helm signal이 없습니다.</p>";
  return `<ul>${items.map((item) => `<li><strong>${escapeHtml(item[labelKey])}</strong> [${escapeHtml(item.readiness)}]<br>${escapeHtml(item.evidence)}<br><a href="${escapeHtml(helmReadinessHref(item.relatedHref))}">관련 페이지 열기</a></li>`).join("")}</ul>`;
}

function helmReadinessCommandList(items: HelmReadinessReport["recommendedCommands"]): string {
  if (items.length === 0) return "<p class=\"muted\">recommended command가 없습니다.</p>";
  return `<ul>${items.map((item) => `<li><code>${escapeHtml(item.command)}</code><br>${escapeHtml(item.purpose)}</li>`).join("")}</ul>`;
}

function helmReadinessRiskList(items: HelmReadinessReport["riskQueue"]): string {
  if (items.length === 0) return "<p class=\"muted\">risk queue가 없습니다.</p>";
  return `<ul>${items.map((item) => `<li><strong>${escapeHtml(item.priority)}</strong>: ${escapeHtml(item.action)}<br><span class="muted">${escapeHtml(item.why)}</span><br><a href="${escapeHtml(helmReadinessHref(item.relatedHref))}">관련 페이지 열기</a></li>`).join("")}</ul>`;
}
...
function apiGatewayReadinessSetupList(items: ApiGatewayReadinessReport["apiGatewaySetups"]): string {
  if (items.length === 0) return "<p class=\"muted\">API gateway setup이 없습니다.</p>";
  return `<ul>${items.map((item) => `<li><strong>${escapeHtml(item.filePath)}</strong> [${escapeHtml(item.gateway)}/${escapeHtml(item.readiness)}]<br>routes/upstreams/auth/plugins ${item.routeCount}/${item.upstreamCount}/${item.authCount}/${item.pluginCount}<br>traffic/transform/observability/validation/deployment/CI ${item.trafficPolicyCount}/${item.transformCount}/${item.observabilityCount}/${item.validationCount}/${item.deploymentCount}/${item.ciCount}<br>${escapeHtml(item.evidence)}<br><a href="${escapeHtml(apiGatewayReadinessHref(item.sourceHref))}">원본 열기</a></li>`).join("")}</ul>`;
}

function apiGatewayReadinessSignalList<T extends string>(items: Array<Record<T, string> & { readiness: string; evidence: string; relatedHref: string }>, labelKey: T): string {
  if (items.length === 0) return "<p class=\"muted\">API gateway signal이 없습니다.</p>";
  return `<ul>${items.map((item) => `<li><strong>${escapeHtml(item[labelKey])}</strong> [${escapeHtml(item.readiness)}]<br>${escapeHtml(item.evidence)}<br><a href="${escapeHtml(apiGatewayReadinessHref(item.relatedHref))}">관련 페이지 열기</a></li>`).join("")}</ul>`;
}

function apiGatewayReadinessCommandList(items: ApiGatewayReadinessReport["recommendedCommands"]): string {
  if (items.length === 0) return "<p class=\"muted\">recommended command가 없습니다.</p>";
  return `<ul>${items.map((item) => `<li><code>${escapeHtml(item.command)}</code><br>${escapeHtml(item.purpose)}</li>`).join("")}</ul>`;
}

function apiGatewayReadinessRiskList(items: ApiGatewayReadinessReport["riskQueue"]): string {
  if (items.length === 0) return "<p class=\"muted\">risk queue가 없습니다.</p>";
  return `<ul>${items.map((item) => `<li><strong>${escapeHtml(item.priority)}</strong>: ${escapeHtml(item.action)}<br><span class="muted">${escapeHtml(item.why)}</span><br><a href="${escapeHtml(apiGatewayReadinessHref(item.relatedHref))}">관련 페이지 열기</a></li>`).join("")}</ul>`;
}

function apiGatewayReadinessHref(href: string): string {
  if (href.startsWith("source/")) return `../${href}`;
  return htmlPageHref(href);
}
