...
export function renderStudyHtml(input: StudyHtmlInput): RenderedStudy {
  ...
  const pages: RenderedPage[] = [
    ...
    },
    {
      name: "quiz.html",
      title: "퀴즈",
      html: pageShell("퀴즈", "quiz.html", `<section class="cards">${input.quiz.questions.map((question, index) => `<article id="${question.id}"><h3>${index + 1}. ${escapeHtml(question.question)}</h3>${Object.entries(question.choices).map(([key, value]) => `<p><strong>${key}</strong>. ${escapeHtml(value)}</p>`).join("")}<details><summary>정답과 해설</summary><p>${escapeHtml(question.correctChoice)}: ${escapeHtml(question.explanation)}</p></details></article>`).join("")}</section>`, input)
    },
    {
