...
function help(): void {
  ...
  open <session-id-or-path> --target verification|evidence|suggested-reads|runtime-environment|interface-map|symbol-map|api-reference|search-index|learning-journal|project-activity|license-rights|sbom|security-readiness|scorecard|provenance|advisories|vex|policy-gates|api-contracts|observability|performance|e2e|accessibility|storybook|design-tokens|i18n|release-readiness|secret-readiness|container-readiness|context-pack|mcp-handoff|agent-memory|graph-query|tutorial-abstractions|decision-records|dependency-health|learning-path|quiz|quiz-print|all --format json|markdown
